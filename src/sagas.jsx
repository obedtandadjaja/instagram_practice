    ...
    filepicker.pick(
      {
        ...
        container: 'modal',
        services: [
         'COMPUTER', 'FACEBOOK', 'INSTAGRAM', 'URL', 'IMGUR', 'PICASA',
        ],
        openTo: 'COMPUTER',
      },
      function(Blob) {
...
function* getProfileInfo() {
  try {
    const profileList = yield getProfileFromServer();
    yield put({
      type: 'GET_PROFILES_SUCCESS',
      payload: profileList,
    });
  } catch (error) {
    yield put({type: 'GET_PROFILES_FAILURE'});
  }
}
...
function* postImage() {
  try {
    ...
    const url = FILESTACK_URL +
                state.get('upload').get('filters') +
                state.get('upload').get('handle');
...
function* watchGetProfiles () {
  yield takeLatest('GET_PROFILES', getProfileInfo);
}
